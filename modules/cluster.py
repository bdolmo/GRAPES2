...
def launch_sample_clustering(sample_list, analysis_dict):
    '''
    '''
    sample_list, analysis_dict = calculate_depth_correlation(sample_list,
        analysis_dict)
...
def cluster_samples(corr_tsv, sample_list, min_correlation=0.91, min_refs=1):
    ...
    with open(corr_tsv) as f:
        for line in f:
            ...
            # data
                ...
                corr_dict[current_sample]['mean_correlation'] = round(np.mean(tmp_corr_list),3)
    ...
    for sample in sample_list:
        ...
        ref_dict = sorted(corr_dict[sample_name]['correlations'].items(), key=lambda item: item[1], reverse=True)
...
def create_heatmap(sample_list, analysis_dict):
    ...
    correlation_plot = str(Path(analysis_dict['output_dir'])/"correlation.heatmap.png")
