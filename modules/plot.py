...
def plot_normalization(sample_list, analysis_dict):
    ...
    for sample in sample_list:
        ...
        if not os.path.isfile(normalization_plot):
            ...
            axes[0, 0] = sns.boxplot(ax=axes[0, 0], x="gc_integer", y=sample.name,
            ...
            axes[0, 1] = sns.boxplot(ax=axes[0, 1], x="gc_integer", y=normalized_gc_tag,
                data=df, showfliers=False, palette="Blues")
            ...
            axes[1, 0] = sns.boxplot(ax=axes[1, 0],x="map_integer", y=sample.name,
                data=df, showfliers=False, palette="Blues")
            ...
            axes[1, 1] = sns.boxplot(ax=axes[1, 1],x="map_integer", y=normalized_map_tag,
                data=df, showfliers=False, palette="Blues")
...
def plot_gene(sample, sample_list, gene, analysis_dict):
    ...
    plot_dict = {
        'controls': controls_ratios,
        'exons': exons_controls
    }
    sample_dict = dict()
    ...
    with sns.axes_style("dark"):
        ...
        axes = sns.boxplot(x="exons", y="controls", data=plot_df, showfliers=False,color="#d4ebf2").set_title(gene)
        axes = sns.scatterplot(x="exons", y=sample_tag, data=sample_df, s=140, color="red")
...
class CnvPlot:
    ...
    '''
    def __init__(self, cnr_file, cns_file, calls, sample, output_dir, dup_cutoff, del_cutoff):
        # ratios
        ...
        if genomewide == True:
            if not os.path.isfile(plot):
                ...
                ratio_plot = sns.scatterplot(data=cnr_df, x=cnr_df.index,
                    y=cnr_df[sample_ratio], size=0.1, hue=cnr_df.chr,
                    palette=palette_dict, edgecolor="none")
