...
def launch_read_depth(sample_list, analysis_dict, ngs_utils, ann_dict):
    '''
    '''
    sample_list, analysis_dict = extract_read_depth(sample_list, analysis_dict,
        ngs_utils, ann_dict)
...
def unify_read_depths(sample_list, analysis_dict):
    ...
    '''
    df_list = []
    df_ready_bed = pd.read_csv(analysis_dict['ready_bed'],header=0, sep = '\t',
        names=['chr', 'start', 'end', 'region', 'gc', 'map'])

    for sample in sample_list:
        df = pd.read_csv(sample.region_coverage, compression='gzip',
            header=0, sep = '\t', names=['chr', 'start', 'end', 'region', sample.name])
        df['gc']  = df_ready_bed['gc']
        df['map'] = df_ready_bed['map']
        df = df[['chr', 'start', 'end', 'region', 'gc', 'map', sample.name]]
        df_list.append(df)

    merge = partial(pd.merge, on=['chr', 'start', 'end', 'region', 'gc', 'map'], how='outer')
    merged_df = reduce(merge, df_list)
    merged_df = merged_df[(df['end']-df['start']) > 10]
    unified_depth_name = ("{}.region.read.depth.bed").format(analysis_dict['output_name'])
    unified_raw_depth = str(Path(analysis_dict['output_dir']) / unified_depth_name)
...
def extract_read_depth_exome(sample_list, analysis_dict, ngs_utils_dict, ann_dict):
    ...
    for sample in sample_list:
        ...
        mosdepth_summary_name  = ("{}{}").format(sample.name,
            ".mosdepth.summary.txt")
        mosdepth_region_name = ("{}{}").format(sample.name,
            ".regions.bed.gz")
        mosdepth_per_base_name = ("{}{}").format(sample.name,
            ".per-base.bed.gz")
        ...
        cmd = ('{} --fast-mode --by {} {} {}') .format(ngs_utils_dict['mosdepth'],\
            analysis_dict['ready_bed'], mosdepth_output, sample.bam)
        ...
        if not os.path.isfile(sample_per_base_file):
            ...
            msg = (" INFO: Skipping coverage extraction from sample {}").format(sample.name)
            logging.info(msg)
